...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture 13/manhattan.csv")
```
...
- Population: 1 bedroom apts in Manhatten 
...
there is 95% confidence that the mean is between 2302 and 2974
...
We're 95% confident that the true proportion of mice who died while not on the 
treatment is between 69% and 79.3%
...
boot_prop_survive <-mice %>% 
  filter(treat ==1) %>% 
  mutate(survive = if_else(died == 0, "yes", "no")) %>% 
  specify(response = survive, success = "yes") %>%
  generate(reps = 10000, type = "bootstrap") %>% 
  calculate(stat = "prop")
```

```{r practice_2_ci}
ci_mice_survive <- get_ci(boot_prop, level = .9)
ci_mice_survive
```
